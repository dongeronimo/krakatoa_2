...
TsdfVolume::TsdfVolume(VkDevice device,
                       VmaAllocator allocator,
                       CommandPoolManager& cmdManager,
                       const std::string& name)
    : device(device), allocator(allocator)
{
    // --- Create the 3D image (256³, R32_UINT, device-local) ---
    ...
    // --- Initialize: transition, fill with INITIAL_PACKED_VALUE, transition to GENERAL ---
    // We use a staging buffer + vkCmdCopyBufferToImage because vkCmdClearColorImage
    // may not produce the exact bit pattern we need on all drivers for R32_UINT.
    // Actually, vkCmdClearColorImage with uint clear values should work fine for R32_UINT.
    cmdManager.SubmitOneShot(CommandPoolManager::QueueType::Graphics,
        [&](VkCommandBuffer cmd) {
            ...
            // Clear to INITIAL_PACKED_VALUE (tsdf=1.0, weight=0)
            // For R32_UINT, the clear value goes in uint32[0]
            VkClearColorValue clearValue{};
            clearValue.uint32[0] = INITIAL_PACKED_VALUE;
            ...
            // TRANSFER_DST → GENERAL (ready for compute storage use)
    ...
    uint32_t sizeMB = (VOLUME_SIZE * VOLUME_SIZE * VOLUME_SIZE * 4) / (1024 * 1024);
    LOGI("TsdfVolume created: %u³ R32_UINT (~%u MB)", VOLUME_SIZE, sizeMB);
}
